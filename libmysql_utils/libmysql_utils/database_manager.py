...
#    from libmysql.database_manager import databaseBackup
#    event = databaseBackup()
...
def event_alter_template_stock():
    from libmysql_utils.mysql8 import ROOT_HEADER
    event = StockEventBase(ROOT_HEADER)
    stock_list = event.get_all_stock_list()
    for stock_code in stock_list:
        print(stock_code)
        sql = f"alter table {stock_code} change highest_price high_price float"
        event.mysql.engine.execute(sql)
        sql = f"alter table {stock_code} change lowest_price low_price float"
        event.mysql.engine.execute(sql)
