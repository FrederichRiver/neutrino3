...
class taskManager2(BackgroundScheduler):
    ...
    def check_task_list(self):
        """
        Set flag for tasks. 'add' for new task, 'changed' for exist task.
        """
    ...
    def task_manage(self, task_list):
        for task in task_list:
            if task.flag == 'add':
                self.add_job(task.func, trigger=task.trigger, id=task.name, timzone=pytz.timezone('Asia/Shanghai'))
            elif task.flag == 'update':
                self.reschedule_job(task.name, trigger=task.trigger, timzone=pytz.timezone('Asia/Shanghai'))
